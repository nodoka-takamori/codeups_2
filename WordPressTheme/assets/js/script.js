...
jQuery(function ($) {
  ...
  //blogページのアーカイブトグル
  document.querySelectorAll(".js-archive__toggle").forEach(function (item) {
    ...
    item.addEventListener("click", function (event) {
      ...
      // 月別アイテムのトグル処理
      var currentItem = item.nextElementSibling;
      while (currentItem && currentItem.classList.contains("aside__archive-item--month")) {
  ...
  var modalOpenElements = document.querySelectorAll(".gallery__photo");
  var modalClose = document.getElementById("js-modal__close");
  ...
  // 各画像クリックでモーダルを表示
  modalOpenElements.forEach(function (item) {
    item.addEventListener("click", function () {
      var imgSrc = item.querySelector("img").src; // クリックされた画像のソースを取得
...
document.addEventListener("DOMContentLoaded", function () {
  var tabs = document.querySelectorAll(".tab__name");
  var contents = document.querySelectorAll(".tab__content");
  var HEADER_HEIGHT = 90; // ヘッダーの高さを設定

  // タブをアクティブにする関数
  function activateTab(index) {
    document.querySelector(".tab__name.is-change").classList.remove("is-change");
  ...
  function scrollToTab(index) {
    var tabPosition = tabs[index].getBoundingClientRect().top + window.pageYOffset - HEADER_HEIGHT;
    window.scrollTo({
      top: tabPosition,
      behavior: "smooth"
    });
  ...
  function checkHash() {
    ...
    if (hash === "#tab3") {
      activateTab(2);
      setTimeout(function () {
        return scrollToTab(2);
      }, 1); // 遅延でスクロール実行 ここがチカっとする原因 これを消すとスクロールされない
    } else if (hash === "#tab1") {
      activateTab(0);
      setTimeout(function () {
        return scrollToTab(0);
      }, 1);
    } else if (hash === "#tab2") {
      activateTab(1);
      setTimeout(function () {
        return scrollToTab(1);
      }, 1);
    }
  }
